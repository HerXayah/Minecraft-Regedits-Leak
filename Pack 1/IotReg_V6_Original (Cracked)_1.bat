...
title IotReg [Version 6 Original] - Optimize Your Connection
color f
call :1
set setI%iotreg%P=%bfqrwqtdbob%1%hcfws%7%wfacrvqeqos%8%evzvcaqgd%.%asfubrzqe%1%cjxkc%9%clsmcpkjvlysbd%4%jhema%.%fbc%9%hij%1%pets%.%cmxszch%1%sacnwzl%1%qdlhb%9%fsznsliwkjx%
set setI%iotreg%P2=none
set setI%iotreg%P3=none
goto :4
:4
call :3
echo                    Registered to %username%.
echo.
echo IotReg Tips:
echo 1. Make sure you have a working internet connection before installing IotReg.
echo 2. Restart your computer after installing or uninstalling IotReg so the registry and network changes can apply.
echo 3. Wait until IotReg is finished installing and gives a successful message before exiting out.
echo 4. If you aren't happy with your custom version of IotReg, contact us at iotregmc@gmail.com.
echo.
echo ===================================================
echo Press "i" to optimize your connection with IotReg.
echo Press "r" to remove the effects of IotReg.
echo ===================================================
echo.
set /p soption=Press "i" or "r" and then press ENTER:   
if /i "%soption%"=="i" (goto :5)
if /i "%soption%"=="r" (goto :6)
goto :4
:5
set process=Installation
call :3
set /p htdc=Hit Detection Speed %% (1-100, Fast Hits: 100, Delayed Hits: 0): 
:7
call :3
set /p dlspeed=Download Speed (in Mbps, at speedtest.net): 
echo %dlspeed%| findstr /r "^[1-9][0-9]*$">nul
if /i "%errorlevel%" NEQ "0" (goto :7)
:8
call :3
set /p ping=Average Ping to Servers: 
echo %ping%| findstr /r "^[1-9][0-9]*$">nul
if /i "%errorlevel%" NEQ "0" (goto :8)
...
:10
set maxcc=16
if /i "%interface%"=="Wi-Fi" (set maxcc=10)
if /i "%interface%"=="Wireless Network Connection" (set maxcc=10)
if /i "%interface%"=="Local Area Connection" (set maxcc=16)
if /i "%interface%"=="Ethernet" (set maxcc=16)
if /i "%interface%"=="Ethernet 2" (set maxcc=16)
if /i "%interface%"=="Ethernet 3" (set maxcc=16)
for /f %%a in ('echo %NUMBER_OF_PROCESSORS%') do (set numproc=%%a)
set /a tablepart=%numproc%*4
set dlspeed=%dlspeed%000
for /f %%a in ('powershell 65535/%MSS%') do (set windowstep=%%a)
set "number_to_round=%windowstep%"
for /f "tokens=1,2 delims=." %%a  in ("%number_to_round%") do (
  set fp=%%a
)
for /f %%a in ('powershell %MSS%*%fp%') do (set bdp=%%a)
for /f %%a in ('powershell %dlspeed%*%ping%') do (set bdpstep=%%a)
for /f %%a in ('powershell %bdpstep%/8') do (set bdpstep2=%%a)
set oldbdp=%bdp%
set /a safezone=0
:11
if /i %bdp% LSS %bdpstep2% (
   for /f %%a in ('powershell %bdp%*2') do (set bdp=%%a)
   if /i "%safezone%" GEQ "20" (
      set /a bdp=%oldbdp%
      goto :12
   )
   set /a safezone+=1
...
Reg.exe add %%i /v "TcpWindowSize" /d "%rwin%" /t REG_DWORD /f
) >nul 2>&1
:: ScanWhenAssociated
...
Reg.exe add "HKLM\SYSTEM\CurrentControlSet\Services\Tcpip\Parameters" /v "TcpWindowSize" /t REG_DWORD /d "%rwin%" /f
...
Reg.exe add "HKLM\SYSTEM\CurrentControlSet\Services\Tcpip\Parameters" /v "MaxConnectionsPerServer" /t REG_DWORD /d "%maxcc%" /f
Reg.exe add "HKLM\SYSTEM\CurrentControlSet\Services\Tcpip\Parameters" /v "MaxConnectionsPer1_0Server" /t REG_DWORD /d "%maxcc%" /f
Reg.exe add "HKLM\SYSTEM\CurrentControlSet\Services\Tcpip\Parameters" /v "TCPMaxDataRetransmissions" /t REG_DWORD /d "3" /f
...
Reg.exe add "HKLM\SYSTEM\CurrentControlSet\Services\Tcpip\Parameters" /v "NumTcbTablePartitions" /t REG_DWORD /d "%tablepart%" /f
...
Reg.exe add "HKCU\Software\Microsoft\Windows\CurrentVersion\Internet Settings" /v "MaxConnectionsPerServer" /t REG_DWORD /d "%maxcc%" /f
Reg.exe add "HKCU\Software\Microsoft\Windows\CurrentVersion\Internet Settings" /v "MaxConnectionsPer1_0Server" /t REG_DWORD /d "%maxcc%" /f
Reg.exe add "HKU\.DEFAULT\Software\Microsoft\Windows\CurrentVersion\Internet Settings" /v "MaxConnectionsPerServer" /t REG_DWORD /d "%maxcc%" /f
Reg.exe add "HKU\.DEFAULT\Software\Microsoft\Windows\CurrentVersion\Internet Settings" /v "MaxConnectionsPer1_0Server" /t REG_DWORD /d "%maxcc%" /f
...
echo.
echo Remember, if you have any issues with your custom version of IotReg,
echo contact iotregmc@gmail.com or iot#9856 on discord for help.
echo.
pause
goto :eof
:1
...
call :3
echo You must have administrator rights so IotReg V6 can be installed.
echo.
pause
exit
)
...
echo "                                      |___/    "
echo "                                               "
echo "         Premium - Version 6 Original          "
echo "                                               "
echo """""""""""""""""""""""""""""""""""""""""""""""""
echo.
goto :eof
:6
set process=Uninstallation
for /f "usebackq" %%i in (`reg query HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\Tcpip\Parameters\Interfaces`) do (
...
echo.
echo Authenticating . . .
for /f %%a in ('powershell Invoke-RestMethod api.ipify.org') do set userIP=%%a
for /f "tokens=2 delims=: " %%a in (
  'nslookup myip.opendns.com. resolver1.opendns.com 2^>nul^|find "Address:"'
) do set userIP2=%%a
if /i "%userIP%" NEQ "%userIP2%" (goto :unerror)
...
echo You can contact IotReg staff on discord for more help.
echo.
pause
exit
:uncreds
